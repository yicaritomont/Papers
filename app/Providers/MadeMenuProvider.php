...
class MadeMenuProvider extends ServiceProvider
{
    ...
    {	
        $modules = Modulo::where('state',1)->whereIn('id',Menu::select('modulo_id')->get())->get();         
        return $modules;
    }

    public static function get_item_modules($modules)
    {
        $items = Menu::where('modulo_id',$modules)->where('url','<>','.')->where('state',1)->whereRaw('id = menu_id')->get();
        return $items;
    }
    ...
    {
        $child = Menu::where('menu_id',$item)->get();
        return $child;
    }
}
