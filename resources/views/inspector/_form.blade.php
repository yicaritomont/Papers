...
<div class="form-group @if ($errors->has('identification')) has-error @endif">
    ...
    {!! Form::select('profession_id',$professions,null, array('class' => 'input-body', 'required','id' => 'profesion_inspector')) !!}
    @if ($errors->has('profession')) <p class="help-block">{{ $errors->first('profession') }}</p> @endif
</div>
<div class="form-group @if ($errors->has('name')) has-error @endif">
    <label for="name">@lang('words.InspectorType')</label>
    {!! Form::select('inspector_type_id',$inspector_types,null, array('class' => 'input-body','require','id' => 'tipo_inspector')) !!}
